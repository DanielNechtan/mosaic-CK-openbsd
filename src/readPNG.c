...
/* Archaic: png_info struct removed from png.h from v1.5.0
*  so don't do this:   *width = (int)png_ptr->width; 
*/
...
/* dithering was disabled from libpng 1.4.0 because it's fucked */
...
#ifndef DISABLE_TRACE
            if (srcTrace) {
                fprintf(stderr,"dithering (RGB->file supplied palette) was disabled in libpng 1.4.0\n");
            }
#endif
...
#ifndef DISABLE_TRACE
            if (srcTrace) {
                fprintf(stderr,"screen wgamma=%f\n",screen_gamma);
            }
#endif
...
/*                    printf("setting gamma=%f\n",info_ptr->gamma);*/
	printf("setting gamma=%f\n",(double)png_get_gAMA(png_ptr, info_ptr, &file_gamma));
                }
#endif
